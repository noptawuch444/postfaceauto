        ...
        <div className="gs-post-form-section" style={{
            ...
            <div className="gs-card-body-scroll" style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    ...
                    <div>
                        ...
                        {imagePreviews.length === 0 ? (
                            <div onClick={() => document.getElementById('img-up').click()}
                                ...
                                onMouseOver={e => e.currentTarget.style.borderColor = V.pri}
                                onMouseOut={e => e.currentTarget.style.borderColor = 'rgba(201, 168, 76, 0.35)'}>
                            ...
                            <div className="gs-image-scroller" style={{ display: 'flex', gap: '10px', overflowX: 'auto', paddingBottom: '10px' }}>
                                <div onClick={() => document.getElementById('img-up').click()} style={{ width: '100px', aspectRatio: '1', borderRadius: '12px', border: `1.5px dashed rgba(201,168,76,0.3)`, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    ...
                    <div style={{ display: 'flex', background: V.bgMain, borderRadius: '10px', border: `1px solid ${V.bdr}`, overflow: 'hidden' }}>
                        <button type="button" onClick={() => setPostNow(true)} style={{ flex: 1, padding: '12px', border: 'none', fontSize: '13px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', background: postNow ? `linear-gradient(135deg, ${V.priD}, ${V.pri})` : 'transparent', color: postNow ? '#1a1200' : V.txtS }}><Send size={14} /> โพสต์ทันที</button>
                        <button type="button" onClick={() => setPostNow(false)} style={{ flex: 1, padding: '12px', border: 'none', borderLeft: `1px solid ${V.bdr}`, fontSize: '13px', fontWeight: '700', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', background: !postNow ? `linear-gradient(135deg, ${V.priD}, ${V.pri})` : 'transparent', color: !postNow ? '#1a1200' : V.txtS }}><Clock size={14} /> ตั้งเวลาโพสต์</button>
                    </div>
                        ...
                        <div style={{ animation: 'fadeSlideIn 0.3s ease-out', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                            ...
                            <div style={{ position: 'relative', height: '54px' }}>
                                <div style={{ height: '100%', background: '#0a0a0a', border: `1.5px solid ${V.pri}`, borderRadius: '12px', display: 'flex', alignItems: 'center', padding: '0 12px', gap: '6px', boxSizing: 'border-box' }}>
                                    ...
                                    <input type="text" placeholder="13:00" value={splitTime.time} onChange={handleTimeChange} style={{ background: 'none', border: 'none', color: '#fff', fontSize: '18px', fontWeight: '800', width: '100%', outline: 'none', fontFamily: 'inherit', letterSpacing: '1px' }} />
                    ...
                    {isAutoReplyEnabledInTemplate && (
                        <div style={{ padding: '14px 16px', background: 'rgba(201,168,76,0.04)', borderRadius: '12px', border: '1px solid rgba(201,168,76,0.15)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: autoReplyEnabled ? '12px' : '0' }}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><span style={{ fontSize: '16px' }}>🤖</span><span style={{ fontSize: '13px', fontWeight: '700', color: '#e2c97e' }}>ตอบกลับอัตโนมัติ</span></div>
                                <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px', cursor: 'pointer' }}>
                                    ...
                                    <span style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: autoReplyEnabled ? '#c9a84c' : 'rgba(255,255,255,0.1)', borderRadius: '24px', transition: '0.3s' }}><span style={{ position: 'absolute', height: '18px', width: '18px', left: autoReplyEnabled ? '23px' : '3px', bottom: '3px', backgroundColor: 'white', borderRadius: '50%', transition: '0.3s' }} /></span>
                                </label>
                            </div>
                            {autoReplyEnabled && <textarea rows={3} placeholder="ข้อความตอบกลับ..." value={autoReplyText} onChange={e => setAutoReplyText(e.target.value)} style={{ ...inputStyle, fontSize: '13px', resize: 'none', padding: '12px' }} onFocus={focusIn} onBlur={focusOut} />}
                        </div>
                    )}
            ...
            {/* Fixed Footer aligned with card border */}
            <div className="gs-card-footer" style={{
                padding: '16px 20px', borderTop: `1px solid ${V.bdr}`, background: 'rgba(0,0,0,0.15)',
                display: 'grid', gridTemplateColumns: 'minmax(0, 1.5fr) minmax(0, 1fr) minmax(0, 1fr)', gap: '8px', flexShrink: 0
            }}>
                <button type="button" onClick={handleSubmit} disabled={loading} style={{ padding: '12px', borderRadius: '8px', fontSize: '14px', fontWeight: '700', border: 'none', background: V.pri, color: '#1a1200', cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
