...
// Note: Config is retrieved via functions.config()
const getAppConfig = () => {
    return {
        appId: process.env.FACEBOOK_APP_ID || functions.config().facebook?.app_id,
        appSecret: process.env.FACEBOOK_APP_SECRET || functions.config().facebook?.app_secret
    };
};
...
async function exchangeCodeForToken(code, redirectUri) {
    ...
    const res = await fetch(`${FB_GRAPH}/oauth/access_token?client_id=${config.appId}&client_secret=${config.appSecret}&redirect_uri=${encodeURIComponent(redirectUri)}&code=${code}`);
...
// Subscribe a page to the webhook
